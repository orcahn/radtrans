...
class RadiativeTransfer:
    ...
    def main(self,argv):
        ...
        boundary_values = [float(e.strip()) for e in config.get('Model', 'BoundaryValues').split(',')]
        quadrature_weights = [float(e.strip()) for e in config.get('Model', 'QuadratureWeights').split(',')]
        ...
        model_problem = modelProblem.ModelProblem1d(temperature, frequency, albedo, scattering,                 domain, boundary_values)
        assert(method == "FiniteVolume")
        if scattering == "isotropic":
            disc = discretization.FiniteVolume1d(model_problem,n_cells,quadrature_weights)
        else:
